...
class EDMNoiseScheduler:
    def __init__(
        ...
        self.normalDistribution = torch.distributions.Normal(loc=self.pMean,scale=self.pSTD)
        #Uniform Distribution 
        self.uniformDistribution = torch.distributions.Uniform(low=self.uniformLower,high=self.uniformUpper)
    ...
    def EDMScalingFactors(self,sigma:Tensor) -> tuple[Tensor,Tensor,Tensor,Tensor]:
        batch = sigma.shape[0]
        unitSigma = sigma**2 + self.sigmaData**2
        
        cinSigma = 1/ (torch.sqrt(unitSigma))
        noiseSigma = .25 * torch.log(sigma)
        
        outputSigma = (sigma * self.sigmaData) / torch.sqrt(unitSigma)
        
        skipSigma = (self.sigmaData**2) / (unitSigma)
        
        return skipSigma,outputSigma, cinSigma, noiseSigma

    
def buildInferenceScheduler(device: torch.device, numSteps:int) -> FlowUniPCMultistepScheduler:
