...
from cosmos_predict2._src.predict2.cosmos_policy.modules.cosmos_sampler import (
    CosmosPolicySampler,
)
...
class CosmosInferenceSolverEDM:
    def __init__(self, cfg: DictConfig, diffusionModel, device):
        ...
        self.device = device
        self.noiseScheduler = EDMNoiseScheduler(cfg=cfg, inference=True, device=device)
    ...
    def denoiser(
        self, noisyNetworkInput, sigma, crossAttentionEmbed, conditioningMasks
    ):
    ...
    def runSolver(
        ...
        pureNoise = (
            torch.randn_like(builtVAEInput) * self.sigmaMax
        )  # test tomorrow and see if it works or not.
        noisyInput = (
            conditioningMasks * builtVAEInput + (1 - conditioningMasks) * pureNoise
        )

        def _denoiser(x, sigma):
            predictedX0 = self.denoiser(
                x, sigma, crossAttentionEmbed, conditioningMasks
            )

            lockedCondition = (
                conditioningMasks * builtVAEInput
                + (1 - conditioningMasks) * predictedX0
            )
...
class CosmosInferenceSolverRecFlow:
    def __init__(self, cfg: DictConfig, diffusionModel, device):
        ...
        self.noiseScheduler = RecFlowNoiseScheduler(
            cfg=cfg, inference=True, device=device
        )
...
def worldModelInjector(
    originalVAELatent: torch.Tensor, predictedActionChunk: torch.Tensor
):
...
def valueModelInjector(
    originalVAELatent: torch.Tensor,
    predictedActionChunk: torch.Tensor,
    predictedFutureWristImg: torch.Tensor,
    predictedFutureLeftImg: torch.Tensor,
    predictedFutureRightImg: torch.Tensor,
    predictedFutureProprio,
):
    originalVAELatent = originalVAELatent.clone()
    originalVAELatent[:, :, 5, :, :] = predictedActionChunk.to(originalVAELatent.dtype)
    originalVAELatent[:, :, 6, :, :] = predictedFutureProprio.to(
        originalVAELatent.dtype
    )
    originalVAELatent[:, :, 7, :, :] = predictedFutureWristImg.to(
        originalVAELatent.dtype
    )
    originalVAELatent[:, :, 8, :, :] = predictedFutureLeftImg.to(
        originalVAELatent.dtype
    )
    originalVAELatent[:, :, 9, :, :] = predictedFutureRightImg.to(
        originalVAELatent.dtype
    )
    return originalVAELatent
...
class unNormalizeLatents(torch.nn.Module):
    ...
    def _inverseLatentBlock(self, latent, flattenedLength):
        ...
        recovered = rearrange(
            recovered, "b (n r) -> b n r", n=fullCopies, r=flattenedLength
        )
    ...
    def unnormAction(self, actionChunk):
        ...
        flatReshape = rearrange(
            flat, "b (t d) -> b t d", t=self.actionHorizon, d=self.actionDim
        )
    ...
    def unnormProp(self, propChunk):
        ...
        proprio = (
            propNorm * (self.propMax - self.propMin) + self.propMin # pyrefly:ignore
        )  
