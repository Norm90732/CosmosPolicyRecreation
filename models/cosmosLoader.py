...
class EncoderVAE(torch.nn.Module):
    ...
    def forward(self, x: Float[Tensor, "b rgb t h w"]) -> Float[Tensor, "b 16 latentT latentH latentW"]:
...
class ReasonTextEncoder(torch.nn.Module):
    ...
    @torch.no_grad()
    @jaxtyped(typechecker=beartype)
    def forward(self, prompt: str) -> Float[Tensor, "1 seqlen embedDim"]: #100352 = embedDim
